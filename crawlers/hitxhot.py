...
class HitxhotCrawler(BaseCrawler):
    """Hitxhot爬虫"""
    
    def __init__(self):
        super().__init__('hitxhot')
        self.list_url_template = f"{self.base_url}/hot/page/{{page}}/"
    ...
    def get_galleries(self, page: int = 1) -> Iterator[GalleryItem]:
        ...
        for item in items:
            try:
                ...
                title_elem = item.xpath('.//img/@alt | .//a/@title')
    ...
    def get_images(self, gallery: GalleryItem) -> Iterator[ImageItem]:
        ...
        if not img_urls:
            img_urls = html.xpath('//img[@class="alignnone"]/@src')
...
if __name__ == '__main__':
    ...
    with HitxhotCrawler() as crawler:
