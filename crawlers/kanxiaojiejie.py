...
class KanxiaojiejieCrawler(BaseCrawler):
    """看小姐姐爬虫"""
    
    def __init__(self):
        super().__init__('kanxiaojiejie')
    ...
    def get_galleries(self, page: int = 1) -> Iterator[GalleryItem]:
        ...
        # 解析图集列表
        items = html.xpath('//div[@class="content"]//article')
        
        if not items:
            items = html.xpath('//article')
        ...
        for item in items:
            try:
                ...
                title_elem = item.xpath('.//h2/text() | .//h2/a/text() | .//a/@title')
...
if __name__ == '__main__':
    ...
    with KanxiaojiejieCrawler() as crawler:
