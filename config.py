...
# 下载根目录，可通过环境变量覆盖
DOWNLOAD_ROOT = Path(os.getenv('CRAWLER_DOWNLOAD_PATH', 'C:/crawler_downloads'))
...
DEFAULT_HEADERS = {
    ...
    "Accept-Encoding": "gzip, deflate",  # 移除br，因为requests默认不支持brotli
...
PLAYWRIGHT_CONFIG = {
    ...
    'timeout': 60000,           # 页面加载超时（毫秒）- 增加到60秒
    'wait_until': 'domcontentloaded', # 等待策略: load, domcontentloaded, networkidle
}
